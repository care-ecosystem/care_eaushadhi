...
class EAushadhiService:
    ...
    def process_eaushadhi_response(
        ...
        # deployment = settings.EAUSHADHI_DEPLOYMENT
        logging.info("Using deployment settings for : %s", deployment)
            # Reads from plug_config.py automatically
        
        if deployment.lower() == "karnataka":
            service = KarnatakaResponseService()
        

        logging.info("Current deployment settings is: %s", deployment)
...
# """Updated eAushadhi fetch service using the validator architecture."""

# from care_eaushadhi.api.validators import (
#     BaseResponseValidator,
#     KarnatakaResponseService,  # ← Use the service
#     ValidationError,
# )
...
#         """Validate and map eAushadhi response."""
